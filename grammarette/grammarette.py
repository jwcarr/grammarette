...
class Grammarette:
    def __init__(self, lexicon, dims):
        ...
        self.meanings = [
            tuple(map(str, meaning)) for meaning, signal in lexicon.items()
        ]
    ...
    def _codelength(self, grammar):
        ...
        char_codelengths = {
            char: np.log2(grammar.count(char) / grammar_length) for char in unique_chars
        }
    ...
    def _produce_signal_for_meaning(self, grammar, target_meaning):
        ...
        for rule in grammar.split("+"):
            for sub_rule in rule.split("|"):
                ...
                if all(
                    [
                        m_val == t_val or m_val == "?"
                        for m_val, t_val in zip(meaning, target_meaning)
                    ]
                ):
                    signal += form
                    break
    ...
    def _reduce_meanings(self, chunk_meanings):
        ...
        for feature_i, n_values in enumerate(self.dims):
            values_for_feature_i = set(
                [meaning[feature_i] for meaning in chunk_meanings]
            )
    ...
    def _write_grammar(self, alignment, chunk_boundaries, reduce_sub_grammar=False):
        ...
        for start, end in chunk_boundaries:
            chunks = [
                "".join(chunk).replace("_", "") for chunk in alignment[:, start:end]
            ]
            chunk_to_meanings = {}
    ...
    def _find_shortest_grammar(self):
        ...
        for alignment in [self.alignment, self.alignment_reverse]:
            for chunk_boundaries in _iter_partition_boundaries(len(alignment[0])):
                self._write_grammar(
                    alignment, chunk_boundaries, reduce_sub_grammar=False
                )
                self._write_grammar(
                    alignment, chunk_boundaries, reduce_sub_grammar=True
                )
        ...
        if self._regex is None:
            ...
            for rule in self.shortest_grammar.split("+"):
                ...
                for sub_rule in rule.split("|"):
                    ...
                    form = sub_rule[self.n_features :]
                    meaning_id = (
                        f'{generate_random_group_name()}{meaning.replace("?", "_")}'
                    )
    ...
    def comprehend(self, signal):
        ...
        for meaning in self.meanings:
            if all(
                [
                    m_val == i_val or i_val == "?"
                    for m_val, i_val in zip(meaning, inferred_meaning)
                ]
            ):
...
def _validate_lexicon(lexicon, dims):
    if not isinstance(lexicon, dict):
        raise TypeError(
            f"Lexicon should be of type dict, got {lexicon.__class__.__name__}"
        )
    for meaning, signal in lexicon.items():
        if not isinstance(meaning, tuple):
            raise TypeError(
                f"Meanings should be of type tuple, got {meaning.__class__.__name__}"
            )
        if not isinstance(signal, str):
            raise TypeError(
                f"Signals should be of type str, got {signal.__class__.__name__}"
            )
        if len(meaning) != len(dims):
            raise ValueError(
                f"Meaning {meaning} does not match the dimensionality {dims}"
            )
        for feature_value, n_features in zip(meaning, dims):
            if feature_value >= n_features:
                raise ValueError(
                    f"Meaning {meaning} is not within the dimensionality {dims}"
                )
    return lexicon
...
def _validate_dims(dims):
    ...
    for n_features in dims:
        if not isinstance(n_features, int):
            raise TypeError(
                f"Dims should be tuple of ints, got tuple containing {n_features.__class__.__name__}"
            )
    return dims
