"""
This file was adapted from https://github.com/fbkarsdorp/alignment
"""
...
def needle_wunsch(sequence_a, sequence_b, scorer, gap_penalty=1, scale=1.0):
    ...
    for i in range(1, len1 + 1):
        for j in range(1, len2 + 1):
            gap_a = matrix[i - 1, j] + (
                gap_penalty * scale if pointer[i - 1, j] == UP else gap_penalty
            )
            gap_b = matrix[i, j - 1] + (
                gap_penalty * scale if pointer[i, j - 1] == LEFT else gap_penalty
            )
...
def align_sequences(sequence_a, sequence_b, scoring_fn=None, gap_penalty=1, scale=1.0):
    ...
    scores = {
        (i, j): scoring_fn(sequence_a[i], sequence_b[j])
...
def _align_profiles(
    sequence_a, sequence_b, scoring_fn=None, gap_penalty=1, scale=1.0, gap_weight=0.5
):
    scores = {}
    for i in range(len(sequence_a)):
        for j in range(len(sequence_b)):
...
def multi_sequence_alignment(
    ...
    matrix = pairwise_distances(
        sequences,
        partial(
            align_sequences, scoring_fn=scoring_fn, gap_penalty=gap_penalty, scale=scale
        ),
    )
    ...
    for cluster_id, (node1, node2, _, _) in enumerate(Z, n_seqs):
        ...
        if node1 < n_seqs and node2 < n_seqs:
            align1, align2, _ = align_sequences(
                sequences[node1], sequences[node2], scoring_fn, gap_penalty, scale
            )
        else:
            if node1 < n_seqs:
                sequence_a, sequence_b = [
                    [elt] for elt in sequences[node1]
                ], alignments[node2]
            elif node2 < n_seqs:
                sequence_a, sequence_b = alignments[node1], [
                    [elt] for elt in sequences[node2]
                ]
            else:
                sequence_a, sequence_b = alignments[node1], alignments[node2]
            align1, align2, _ = _align_profiles(
                sequence_a, sequence_b, scoring_fn, gap_penalty, scale, gap_weight
            )
        alignments[cluster_id] = merge(align1, align2)
    return list(zip(*map(flatten, alignments[max(alignments)])))
